...
if (arvish.input.split(" ")[0].toLowerCase() === 'ssh') {
	addOutput(`Type ssh username for ${arvish.input.split(" ")[1]}`, `ssh ${arvish.input.split(" ")[2] || 'root'}@${arvish.input.split(" ")[1]}`, `ssh ${arvish.input.split(" ")[2] || 'root'}@${arvish.input.split(" ")[1]}`, 'Public', 'ssh')
} else if (arvish.input.toLowerCase() === 'scan') {
...
} else if (arvish.input.toLowerCase() === 'ipv6') {
...
Promise.all(promises).then(() => {
	if (!arvish.input) {
		...
		if (process.env['show_contribute'] === 'true') addOutput('Contribute to this workflow on GitHub', 'https://github.com/jeppestaerk/arvis-show-network-info', 'https://github.com/jeppestaerk/arvis-show-network-info#contributions', 'Contribute', 'browser');
	}
